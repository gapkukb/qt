...
import { repeatStrings } from "./support/Strings";
...
export function sourcelikeToSource(sl: Sourcelike): Source {
  ...
  if (typeof sl === "string") {
    ...
    if (lines.length === 1) return { kind: "text", text: sl };
    return {
      kind: "sequence",
      sequence: arrayIntercalate(
        newline(),
        lines.map((l) => ({ kind: "text", text: l } as Source))
      ),
    };
  }
  if (sl instanceof Name) return { kind: "name", named: sl };
  return sl;
}
...
function sourceLineLength(source: Source, names: ReadonlyMap<Name, string>): number {
  switch (source.kind) {
    ...
    case "sequence":
      return source.sequence.map((s: Source) => sourceLineLength(s, names)).reduce((a: number, b: number) => a + b, 0);
    case "table":
...
export function serializeRenderResult(
  ...
  function indentIfNeeded(): void {
    ...
    currentLine.push(repeatStrings(indentation, indentNeeded));
    indentNeeded = 0;
  }
        ...
        for (let y = 0; y < numRows; y++) {
          ...
          for (let x = 0; x < numColumns; x++) {
            ...
            if (x < numColumns - 1 && srcWidth < colWidth) {
              currentLine.push(repeatStrings(" ", colWidth - srcWidth));
            }
          }
          if (y < numRows - 1) {
            finishLine();
...
export function multiWord(separator: Sourcelike, ...words: Sourcelike[]): MultiWord {
  ...
  if (words.length === 1) return singleWord(words[0]);
