...
import { getNonStringTypeMapping, provenanceTypeAttributeKind, StringTypeMapping, TypeBuilder } from "./TypeBuilder";
import { combineTypeAttributesOfTypes, isNamedType, SeparatedNamedTypes, separatedNamedTypes } from "./TypeUtils";
...
export function assertTypeRefGraph(ref: TypeRef, graph: TypeGraph) {
  assert(
    ((ref >> indexBits) & serialMask) === (graph.serial & serialMask),
...
export function derefTypeRef(ref: TypeRef, graphOrBuilder: TypeGraph | BaseGraphRewriteBuilder): Type {
  const graph = getGraph(graphOrBuilder);
  assertTypeRefGraph(ref, graph);
  return graph.typeAtIndex(typeRefIndex(ref));
}
...
export function attributesForTypeRef(
  ...
  const graph = getGraph(graphOrBuilder);
  assertTypeRefGraph(ref, graph);
  return graph.atIndex(typeRefIndex(ref))[1];
}
...
export function typeAndAttributesForTypeRef(
  ...
  const graph = getGraph(graphOrBuilder);
  assertTypeRefGraph(ref, graph);
  return graph.atIndex(typeRefIndex(ref));
}

export class TypeAttributeStore {
  readonly #topLevelValus: Map<string, TypeAttributes> = new Map();
  constructor(private readonly typeGraph: TypeGraph, private values: (TypeAttributes | undefined)[]) {}
  private getTypeIndex(t: Type): number {
    const ref = t.typeRef;
    assertTypeRefGraph(ref, this.typeGraph);
    return typeRefIndex(ref);
  }
  attributesForType(t: Type): TypeAttributes {
    const i = this.getTypeIndex(t);
    const maybe = this.values[i];
    if (maybe) return maybe;
    return emptyTypeAttributes;
  }
  attributesForTopLevel(name: string): TypeAttributes {
    const maybe = this.#topLevelValus.get(name);
    if (maybe) return maybe;
    return emptyTypeAttributes;
  }
  ...
  set<T>(kind: TypeAttributeKind<T>, t: Type, value: T): void {
    ...
    this.values[index] = this.setInMap(this.attributesForType(t), kind, value);
  }
  setForTopLevel<T>(kind: TypeAttributeKind<T>, topLevelName: string, value: T): void {
    this.#topLevelValus.set(topLevelName, this.setInMap(this.attributesForTopLevel(topLevelName), kind, value));
  }
...
export class TypeAttributeStoreView<T> {
  constructor(private readonly attributeStore: TypeAttributeStore, private readonly definition: TypeAttributeKind<T>) {}
  set(t: Type, value: T) {
    this.attributeStore.set(this.definition, t, value);
  }
  setForTopLevel(name: string, value: T) {
    this.attributeStore.setForTopLevel(this.definition, name, value);
  }
  ...
  tryGetForTopLevel(name: string): T | undefined {
    return this.attributeStore.tryGetForTopLevel(this.definition, name);
  }
...
export class TypeGraph {
  ...
  #attributeStore?: TypeAttributeStore;
  #topLevels: Map<string, Type> = new Map();
  #types?: Type[];
  #parent?: Set<Type>[];
  #printOnRewrite: boolean = false;
  constructor(typeBuilder: TypeBuilder, readonly serial: number, private readonly haveProvenanceAttributes: boolean) {
    this.#typeBuilder = typeBuilder;
  }
  private get isFronzen(): boolean {
    return this.#typeBuilder === undefined;
  }
  get attributeStore(): TypeAttributeStore {
    return defined(this.#attributeStore);
  }
  freeze(topLevels: ReadonlyMap<string, TypeRef>, types: Type[], typeAttributes: (TypeAttributes | undefined)[]) {
    assert(!this.isFronzen, "Tried to freeze TypeGraph a second time");
    for (const t of types) {
      assertTypeRefGraph(t.typeRef, this);
    }
    this.#attributeStore = new TypeAttributeStore(this, typeAttributes);
    this.#types = types;
    this.#typeBuilder = undefined;
    this.#topLevels = mapMap(topLevels, (ref) => derefTypeRef(ref, this));
  }
  get topLevels(): ReadonlyMap<string, Type> {
    assert(this.isFronzen, "Cannot get top-levels from a non-frozen graph");
    return this.#topLevels;
  }
  typeAtIndex(index: number): Type {
    if (this.#typeBuilder) return this.#typeBuilder.typeAtIndex(index);
    return defined(this.#types)[index];
  }
  atIndex(index: number): [Type, TypeAttributes] {
    if (this.#typeBuilder) return this.#typeBuilder.atIndex(index);
    const t = this.typeAtIndex(index);
    return [t, defined(this.#attributeStore).attributesForType(t)];
  }
  private filterTypes(predicate: ((t: Type) => boolean) | undefined): ReadonlySet<Type> {
    let seen = new Set<Type>(),
      types: Type[] = [];
    function addFromType(t: Type) {
      if (seen.has(t)) return;
      seen.add(t);
      const required = predicate === undefined || predicate(t);
      if (required) types.push(t);
  ...
  allNamedTypesSeparated(): SeparatedNamedTypes {
    ...
    return separatedNamedTypes(types);
  }
  private allProvenance(): ReadonlySet<number> {
    assert(this.haveProvenanceAttributes);
    ...
    const sets = Array.from(this.allTypesUnordered()).map((t) => {
      const maybe = view.tryGet(t);
      return maybe || new Set();
    });
    const ret: ReadonlySet<number> = new Set();
    setUnionManyInto(ret as any, sets);
    return ret;
  }
  setPrintOnRewrite(): void {
    this.#printOnRewrite = true;
  }
  private checkLostTypeAttributes(builder: BaseGraphRewriteBuilder, newGraph: TypeGraph): void {
    if (!this.haveProvenanceAttributes || builder.lostTypeAttributes) return;
    const oldProvenance = this.allProvenance();
    const newProvenance = newGraph.allProvenance();
    if (oldProvenance.size !== newProvenance.size) {
      const diff = setSubtract(oldProvenance, newProvenance);
      const indexes = Array.from(diff);
      return messageError("IRTypeAttributesNotPropagated", { count: diff.size, indexes });
    }
  }
  private printRewrite(title: string): void {
    if (!this.#printOnRewrite) return;
    console.log(`\n#${title}`);
  }
  rewrite<T extends Type>(
    ...
    if (!force && !replacementGroup.length) return this;
    const builder = new GraphRewriteBuilder(
      this,
      stringTypeMapping,
      alphabetizeProperties,
      this.haveProvenanceAttributes,
      replacementGroup,
  ...
  rewriteFixedPoint(alphabetizeProperties: boolean, debugPrintReconstitution: boolean): TypeGraph {
    ...
    while (true) {
      ...
      if (graph.allTypesUnordered().size === newGraph.allTypesUnordered().size) return graph;
      graph = newGraph;
    }
  }
  allTypesUnordered(): ReadonlySet<Type> {
    assert(this.isFronzen, "Tried to get all graph types before it was frozen");
    return new Set(defined(this.#types));
  }
  makeGraph(invertDirection: boolean, childrenOfType: (t: Type) => ReadonlySet<Type>): Graph<Type> {
    return new Graph(defined(this.#types), invertDirection, childrenOfType);
  }
  getParentsOfType(t: Type): Set<Type> {
    assertTypeRefGraph(t.typeRef, this);
    if (!this.#parent) {
      const parents = defined(this.#types).map((_) => new Set()) as any;
    ...
    return this.#parent![t.index];
  }
  printGraph(): void {
    const types = defined(this.#types);
    for (let i = 0; i < types.length; i++) {
      const t = types[i];
      const parts: string[] = [];
      parts.push(`${t.debugPrintKind}${t.hasNames ? `${t.getCombineName()}` : ""}`);
      ...
      for (const [kind, value] of t.getAttributes()) {
        const maybe = kind.stringify(value);
        if (maybe) parts.push(maybe);
      }
...
export function noneToAny(
  ...
  if (!noneTypes.size) return graph;
  ...
  return graph.rewrite(
    ...
    (types, builder, forwardingRef) => {
      ...
      const ref = builder.getPrimitiveType("any", attributes, forwardingRef);
      return ref;
    }
  );
}

export function optinalToNullable(
  graph: TypeGraph,
  stringTypeMapping: StringTypeMapping,
  debugPrintReconstitution: boolean
): TypeGraph {
  ...
  const repalcementGroups = Array.from(classesWithOptional).map((c) => [c as ClassType]);
  if (classesWithOptional.size === 0) return graph;
  return graph.rewrite(
...
export function removeIndirectionIntersections(
  ...
  for (const t of graph.allTypesUnordered()) {
    ...
    while (current.members.size === 1) {
      ...
      if (seen.has(member)) return panic("There's a cycle of intersection types");
      seen.add(member);
      current = member;
    }
  }
  return graph.remap("remove indirection intersections", stringTypeMapping, false, new Map(), debugPrintRemapping);
}
