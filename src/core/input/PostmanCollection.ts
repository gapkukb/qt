...
export function sourceFromPostmanCollection(
  ...
  function processCollection(c: any): void {
    ...
    if (Array.isArray(c.item)) {
      for (const item of c.item) processCollection(item);
      if (typeof c.info === "object" && typeof c.info.description === "string") descriptions.push(c.info.description);
    }
    if (typeof c.name === "string" && Array.isArray(c.response)) {
      ...
      if (samples.length > 0) {
        ...
        if (typeof c.request === "object") {
          ...
          if (method && typeof url === "object" && url.raw) {
  ...
  if (joinedDescription) description = joinedDescription;
